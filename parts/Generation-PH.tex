...
this algorithm will generate a model or complete the input model by adding the delayed actions that could have realized the state changes observed through the chronogram corresponding to the descritized observation data.
...
A chronogram is the discritized time evolution of the time series data of biological regulatory networks. It is presented by the fonction below: \\

$$\mathds{R}^+ \longmapsto \mathds{N}^+$$
$$ [0,T] \in \mathds{N}^+ \longmapsto \{0,1,..., n\}$$

with $T$ is the size of the chronogram and $n$ is the maximum level of discretization.
