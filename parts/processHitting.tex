...
We propose at \pref{def:semantic}, the semantic of the timed PH that we base on to generate the corresponding model to the given observations. Indeed it means that if there is no action in $\PHa$ that is playable in $s$ the state of the system remains $s$ for all time $t'$ with $t' > t$.% (i.e. steady state).
