...
\caption{Semantics of Process Hitting ($\PH = (\PHs,\PHl,\PHap)$) with timed actions such that $\textsf{state}(\PH, t_i)=s_i$, $ \forall$ $t_0 \leq t_i \leq t_n$, with $t_0$ and $t_n$ are respectively the minimum and the maximum time step in the time series data. We mean by $start(action(h_i))$ that the timed action $h_i$ starts playing so that the system state won't change until $h_i$ finishes after $D_i$ time steps with $delay(h_i)=D_i$ presented here by $finish(action(h_i))$.}
\end{figure}
