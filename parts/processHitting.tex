...
\label{def:semantic}
Let $\PH = (\PHs,\PHl,\PHap)$ be a Process Hitting with timed actions. Let $t$, $t'$ and ($t+D$) be time steps and $\textsf{state}(\PH, t)=s$ be the state of $\PH$ at $t$ with $s \in \PHl$.  Let $H_t^s$ be the set of playable timed actions in $s$ at $t$. If no action is being played at $t$ and $H_t^s \not = \emptyset$, then exactly one action $h \in H$ has to be played. Let $h = \PHfrappedelay{A}{D}{b_i}{b_j}$ so that the semantics of $\PH$ can be expressed as follows:
