...
%If there is no temporized sort $a$ that changes its process from $a_i \in A$ to $a_j \not \in A$ while playing $h$ in $s$
If $h$ is chosen as the action to be played at $s$ then the state of $\PH$ is $s$ during $D$ time steps and at $t+D$
%If $h$ is playable in $s$ during $D$ units of time,
the resulting state after playing $h$ 
