...
\caption{Semantics of Process Hitting ($\PH = (\PHs,\PHl,\PHap)$) with timed-actions such that $\textsf{state}(\PH, t_i)=s_i$, $ \forall t_0 \leq t_i \geq t_n$, and $t_n$ is the maximum time step in the time series data. We mean by $start(action(h_i))$ that the action $h_i$ starts playing so that the system state won't change, from $s_i$ to $s_{i+1}$, until $h_i$ finishes presented here by $finish(action(h_i))$.}
\end{figure}
