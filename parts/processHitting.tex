...
\begin{definition}[Process Hitting with Plural Actions]\label{def:PH}
...
\end{definition}
To model biological networks, we used the PH framework with timed plural actions (\pref{def:PH-timed-plural}). Indeed, in the biological models that we studied we need to present not only the cooperation between the components to influence another one (plural action), but also the time that this action need to take place (delays). 
\begin{definition}[Process Hitting with Timed Plural Actions]
\label{def:PH-timed-plural}
...
Note that if all actions delays are set to 1 it is equivalent to Process Hitting without delays (original PH).
